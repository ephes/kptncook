...
from .models import Recipe as KptnCookRecipe, Image
...
class MealieApiClient:
    ...
    def create_unit(self, unit):
        unit_json = {"id":"", "name":unit, "description":"", "fraction":True, "abbreviation":""}
    ...
    def upload_asset(self, recipe_slug, image: Image):
        ...
        # upload to mealie
        data = {"name": image.name.split(".")[0], "icon": "mdi-file-image", "extension": image.name.split(".")[-1]}
        r = self.post(f"/recipes/{recipe_slug}/assets", data=data, files={"file": download_file})
        r.close()
        download_file.close()
        r.raise_for_status()
    ...
    def enrich_recipe_with_step_images(self, recipe):
        for i in range(len(recipe.recipeInstructions)):
            uploaded_image_name = self.upload_asset(recipe.slug, recipe.recipeInstructions[i].image)
            recipe.recipeInstructions[i].text = self._build_recipestep_text(recipe.id, recipe.recipeInstructions[i].text, uploaded_image_name)
...
def kptncook_to_mealie(kcin: KptnCookRecipe, mealie_client: MealieApiClient, api_key: str = settings.kptncook_api_key) -> RecipeWithImage:
    kwargs = {
        ...
        "imageUrl": kcin.get_image_url(api_key),
        #"tags": ["kptncook"],  # tags do not work atm
        #"tags": [RecipeTag(name="kptncook")],
        "tags": [mealie_client.get_tag_by_name(tag) for tag in ["kptncook"]],
